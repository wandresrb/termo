...
systemd_move_to_new_cgroup(char **cause)
{
	...
	xasprintf(&name, "termo-spawn-" SD_ID128_UUID_FORMAT_STR ".scope",
	...
	pid = getpid();
	parent_pid = getppid();
	xasprintf(&desc, "termo child pane %ld launched by process %ld",
	 ...
	 * "app-termo.slice" if that fails.
	 */
	r = sd_pid_get_user_slice(parent_pid, &slice);
	if (r < 0) {
		slice = xstrdup("app-termo.slice");
	}
