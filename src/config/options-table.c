...
const struct options_table_entry options_table[] = {
	...
	{ .name = "clear-on-attach",
	  ...
	  .text = "Whether to use the alternate screen and clear it when "
		  "a client is attached. When disabled, termo does not "
		  "enter the alternate screen on attach so terminal "
		  "content before termo remains in scrollback."
	},
	...
	{ .name = "default-client-command",
	  ...
	  .text = "Default command to run when termo is run without a command."
	},
	...
	{ .name = "theme",
	  ...
	  .default_num = 0,
	  .text = "Whether termo should detect the terminal theme, use terminal "
	...
	{ .name = "allow-passthrough",
	  ...
	  .text = "Whether applications are allowed to use the escape sequence "
		  "to bypass termo. Can be 'off' (disallowed), 'on' (allowed "
	...
	{ .name = "automatic-rename-format",
	  .type = OPTIONS_TABLE_STRING,
	  .scope = OPTIONS_TABLE_WINDOW,
	  .default_str = "#{?pane_in_mode,[termo],#{pane_current_command}}"
			 "#{?pane_dead,[dead],}",
