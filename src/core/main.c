...
make_label(const char *label, char **cause)
{
	...
	*cause = NULL;
	...
	xasprintf(&base, "%s/termo-%ld", path, (long)uid);
	free(path);
...
main(int argc, char **argv)
{
	...
	/*
	 * termo is a UTF-8 terminal, so if TERMO or TMUX is set, assume UTF-8.
	 ...
	 */
	if (getenv("TERMO") != NULL || getenv("TMUX") != NULL)
		flags |= CLIENT_UTF8;
	else {
		s = getenv("LC_ALL");
		if (s == NULL || *s == '\0')
	 ...
	 * used. Otherwise, $TERMO or $TMUX is checked and if that fails
	 * "default" is used.
	 */
