...
wait_tmux()
{
	...
	while [ "$i" -lt "$timeout" ]; do
		if $TERMO display-message -p -t "$target" '#{window_id}' \
		    >/dev/null 2>&1; then
			return 0
		fi
		sleep 1
...
window_fields()
{
	$TERMO display-message -p -t "$1" '#{window_id} #{window_index}'
}

pane_fields()
{
	$TERMO display-message -p -t "$1" '#{window_id} #{window_index} #{pane_id}'
}
...
$TERMO kill-server 2>/dev/null
$TERMO new-session -d -s subs -x 80 -y 24 || exit 1
sid=$($TERMO display-message -p -t subs '#{session_id}')

mkfifo "$IN"
: >"$OUT"
$TERMO -C attach-session -t subs <"$IN" >"$OUT" 2>&1 &
PID=$!
exec 3>"$IN"
...
i=0
while [ "$i" -lt 6 ]; do
	[ "$($TERMO list-panes -t subs:pane-test | wc -l)" -eq 2 ] && break
	sleep 1
	i=$((i + 1))
done
[ "$i" -lt 6 ] || exit 1
newpane=$($TERMO list-panes -t subs:pane-test -F '#{pane_id}' | tail -n 1)
wait_for "%subscription-changed ap $sid $wid $widx $newpane : $newpane" ||
    exit 1

$TERMO new-session -d -s other || exit 1
$TERMO link-window -s subs:pane-test -t other:1 || exit 1
...
wcount=$($TERMO display-message -p -t subs '#{session_windows}')
