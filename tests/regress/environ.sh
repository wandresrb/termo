...
check_value()
{
	out=$($TERMO show-environment $1 2>&1)
	if [ "$out" != "$2" ]; then
...
out=$($TERMO show-environment -g 2>&1)
...
$TERMO show-environment -gh 2>&1 | grep -q '^LISTHID=3$' || \
...
EOF
CTERMO="$TEST_TERMO -LtestB$$ -f$CONF"
CTMUX="$CTERMO"
$CTERMO kill-server 2>/dev/null
$CTERMO new-session -d -s c -x 80 -y 24 || { rm -f "$CONF"; exit 1; }
out=$($CTERMO show-environment -g CFGVAR 2>&1)
[ "$out" = "CFGVAR=fromconfig" ] || \
	{ echo "config assignment failed: '$out'"; $CTERMO kill-server; rm -f "$CONF"; exit 1; }
out=$($CTERMO show-environment -gh CFGHID 2>&1)
[ "$out" = "CFGHID=hiddencfg" ] || \
	{ echo "config %hidden failed: '$out'"; $CTERMO kill-server; rm -f "$CONF"; exit 1; }
# The %hidden variable is hidden from a plain show.
out=$($CTERMO show-environment -g CFGHID 2>&1)
[ "$out" = "" ] || \
	{ echo "config %hidden not hidden: '$out'"; $CTERMO kill-server; rm -f "$CONF"; exit 1; }
$CTERMO kill-server 2>/dev/null
rm -f "$CONF"
