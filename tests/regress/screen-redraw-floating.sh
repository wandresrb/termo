...
new_scene() {
	$TERMO2 neww -d "sh -c 'printf base; exec sleep 100'" || exit 1
...
fill_base_pattern() {
	$TERMO2 respawnp -k "sh -c 'i=0; while [ \$i -lt 12 ]; do printf \"%02d:abcdefghijklmnopqrstuvwxyz0123456789\n\" \$i; i=\$((i + 1)); done; exec sleep 100'" || exit 1
}

# fill_all_panes: make each pane visibly nonblank.
fill_all_panes() {
	for pane in $($TERMO2 list-panes -F '#{pane_id}'); do
		$TERMO2 respawnp -k -t "$pane" "sh -c 'i=0; while [ \$i -lt 5 ]; do printf \"TILE%02d abcdefghij\n\" \$i; i=\$((i + 1)); done; exec sleep 100'" || exit 1
	done
}
...
$TERMO2 set -g pane-border-format " #{pane_title} " || exit 1
...
new_scene 40 12
$TERMO2 new-pane -x20 -y6 -X8 -Y3 "sh -c 'printf FLOAT; exec sleep 100'" || exit 1
...
# not stable).
new_scene 40 12
$TERMO2 setw pane-border-status top || exit 1
$TERMO2 selectp -t:.0 -T base || exit 1
$TERMO2 new-pane -x20 -y6 -X8 -Y2 -T title \
	"sh -c 'printf FLOAT; exec sleep 100'" || exit 1
compare floating-title

# Larger floating pane with double border lines.
new_scene 40 12
$TERMO2 new-pane -x28 -y8 -X4 -Y1 -B double \
	"sh -c 'printf FLOAT; exec sleep 100'" || exit 1
...
new_scene 40 12
$TERMO2 new-pane -x20 -y6 -X10 -Y3 -B none \
...
new_scene 40 12
$TERMO2 new-pane -x20 -y6 -X30 -Y8 "sh -c 'printf CLIP; exec sleep 100'" || exit 1
...
new_scene 28 8
$TERMO2 new-pane -x16 -y5 -X18 -Y4 "sh -c 'printf OUT; exec sleep 100'" || exit 1
...
new_scene 28 8
$TERMO2 respawnp -k "sh -c 'i=0; while [ \$i -lt 8 ]; do printf \"%02d:abcdefghijklmnopq\n\" \$i; i=\$((i + 1)); done; exec sleep 100'" || exit 1
$TERMO2 new-pane -x10 -y5 -X19 -Y2 "sh -c 'printf EDGE; exec sleep 100'" || exit 1
compare floating-clip-window-edge

# Floating pane clipped at the top-left corner (negative offsets).
new_scene 40 12
$TERMO2 new-pane -x18 -y6 -X-4 -Y-2 "sh -c 'printf TL; exec sleep 100'" || exit 1
...
new_scene 40 12
fill_base_pattern
$TERMO2 new-pane -x20 -y6 -X-10 -Y3 "sh -c 'printf LEFT; exec sleep 100'" || exit 1
compare floating-clip-left

new_scene 40 12
fill_base_pattern
$TERMO2 new-pane -x20 -y6 -X30 -Y3 "sh -c 'printf RIGHT; exec sleep 100'" || exit 1
compare floating-clip-right

new_scene 40 12
fill_base_pattern
$TERMO2 new-pane -x20 -y6 -X10 -Y-3 "sh -c 'printf TOP; exec sleep 100'" || exit 1
compare floating-clip-top

new_scene 40 12
fill_base_pattern
$TERMO2 new-pane -x20 -y6 -X10 -Y9 "sh -c 'printf BOTTOM; exec sleep 100'" || exit 1
...
tile_2x2
fill_all_panes
$TERMO2 new-pane -x16 -y6 -X11 -Y3 "sh -c 'printf FLT; exec sleep 100'" || exit 1
...
tile_2x2
fill_all_panes
$TERMO2 new-pane -x16 -y6 -X11 -Y3 -B double \
...
new_scene 40 12
$TERMO2 new-pane -x16 -y6 -X4 -Y2 "sh -c 'printf AAA; exec sleep 100'" || exit 1
$TERMO2 new-pane -x16 -y6 -X14 -Y6 "sh -c 'printf BBB; exec sleep 100'" || exit 1
...
new_scene 40 12
$TERMO2 setw pane-border-format " #{pane_title} " || exit 1
$TERMO2 new-pane -x16 -y4 -X3 -Y1 -T one \
	"sh -c 'printf ONE; exec sleep 100'" || exit 1
$TERMO2 set -p pane-border-status top || exit 1
$TERMO2 set -p pane-border-lines single || exit 1
$TERMO2 new-pane -x16 -y4 -X18 -Y6 -T two \
	"sh -c 'printf TWO; exec sleep 100'" || exit 1
$TERMO2 set -p pane-border-status bottom || exit 1
$TERMO2 set -p pane-border-lines heavy || exit 1
...
new_scene 40 12
$TERMO2 setw pane-scrollbars on || exit 1
$TERMO2 setw pane-scrollbars-style "bg=black,fg=white,width=1,pad=0" || exit 1
$TERMO2 new-pane -x16 -y6 -X22 -Y3 "sh -c 'printf OVERSB; exec sleep 100'" || exit 1
...
new_scene 40 11
$TERMO2 set status on || exit 1
$TERMO2 set status-position bottom || exit 1
$TERMO2 set status-format[0] "" || exit 1
$TERMO2 new-pane -x20 -y6 -X8 -Y7 "sh -c 'printf OVERST; exec sleep 100'" || exit 1
...
$TERMO2 set -g fill-character '#[acs]~' || exit 1
new_scene 40 12
$TERMO2 new-pane -x20 -y6 -X8 -Y3 "sh -c 'printf FLOAT; exec sleep 100'" || exit 1
...
new_scene 40 12
$TERMO2 breakp -W || exit 1
$TERMO2 set pane-border-lines none || exit 1
