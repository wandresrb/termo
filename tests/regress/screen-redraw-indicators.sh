...
new_scene() {
	$TERMO2 neww -d "sh -c 'exec sleep 100'" || exit 1
...
$TERMO2 new -d -x40 -y12 "sh -c 'exec sleep 100'" || exit 1
...
$TERMO2 set -g pane-border-indicators arrows || exit 1
...
$TERMO2 set -g pane-border-indicators colour || exit 1
...
new_scene
$TERMO2 setw pane-active-border-style fg=red || exit 1
$TERMO2 setw pane-border-style fg=green || exit 1
$TERMO2 splitw -h "$C" || exit 1
$TERMO2 selectp -t0 || exit 1
...
$TERMO2 splitw -h "$C" || exit 1
$TERMO2 selectp -t0 || exit 1
$TERMO2 new-pane -x16 -y6 -X10 -Y2 "$C" || exit 1
$TERMO2 selectp -t0 || exit 1
...
# split also applies.
$TERMO2 set -g pane-border-indicators both || exit 1
new_scene
$TERMO2 setw pane-active-border-style fg=red || exit 1
$TERMO2 setw pane-border-style fg=green || exit 1
$TERMO2 splitw -h "$C" || exit 1
...
$TERMO2 set -g pane-border-indicators off || exit 1

# Left/right split, right pane marked.
new_scene
$TERMO2 splitw -h "$C" || exit 1
$TERMO2 selectp -t0 || exit 1
$TERMO2 selectp -t1 -m || exit 1
...
new_scene
$TERMO2 new-pane -x20 -y6 -X8 -Y3 "$C" || exit 1
$TERMO2 selectp -t0 || exit 1
$TERMO2 selectp -t1 -m || exit 1
...
new_scene
$TERMO2 splitw -h "$C" || exit 1
$TERMO2 setw pane-border-format " #{pane_index}:#{pane_title} " || exit 1
