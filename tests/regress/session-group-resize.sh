...
$TERMO -f/dev/null new -d -s test -x 20 -y 6 || exit 1
$TERMO neww -t test || exit 1
$TERMO selectw -t test:0 || exit 1
...
(echo "refresh-client -C 20,6"; echo "selectw -t :1"; sleep 5) |
	$TERMO -f/dev/null -C attach -t test >$TMP1 2>&1 &

# Wait for small client to be on window 1.
n=0
while [ $n -lt 20 ]; do
	$TERMO lsc -F '#{client_name} #{window_index}' 2>/dev/null | grep -q " 1$" && break
	sleep 0.1
	n=$((n + 1))
done
...
(echo "refresh-client -C 30,10"; echo "switch-client -t :=1"; sleep 5) |
	$TERMO -f/dev/null -C new -t test -x 30 -y 10 >$TMP2 2>&1 &

# Wait briefly for the switch-client command to execute, then check.
# The resize should happen immediately (within 0.2s).
sleep 0.2
OUT1=$($TERMO display -t test:1 -p '#{window_width}x#{window_height}' 2>/dev/null)
...
(echo "refresh-client -C 25,8"; echo "selectw -t :1"; sleep 5) |
	$TERMO -f/dev/null -C new -t test -x 25 -y 8 >$TMP3 2>&1 &

sleep 0.2
OUT2=$($TERMO display -t test:1 -p '#{window_width}x#{window_height}' 2>/dev/null)
