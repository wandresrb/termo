...
{
	fmt=${3:-'#{window_index}'}
	out=$($TERMO display-message -p -t "$1" "$fmt" 2>&1)
	if [ "$out" != "$2" ]; then
...
sid=$($TERMO display-message -p -t alpha: '#{session_id}')
...
wid=$($TERMO display-message -p -t alpha:editing '#{window_id}')
...
w2=$($TERMO display-message -p -t alpha:shell '#{window_id}')
check "alpha:$w2" "2"
bw=$($TERMO display-message -p -t beta: '#{window_id}')
...
check_ok select-window -t alpha:0
pane=$($TERMO display-message -p -t alpha:0 '#{pane_id}')
TERMO_PANE=$pane TMUX_PANE=$pane check "" "alpha" '#{session_name}'	# empty target is current
TERMO_PANE=$pane TMUX_PANE=$pane check "" "alpha:0" '#{session_name}:#{window_index}'
TERMO_PANE=$pane TMUX_PANE=$pane check ":shell" "alpha:2" '#{session_name}:#{window_index}'
check "alpha:shell.0" "alpha:2" '#{session_name}:#{window_index}'
TERMO_PANE=$pane TMUX_PANE=$pane check "alpha:.0" "alpha:0" '#{session_name}:#{window_index}'	# empty window part

# --- bare-name fallbacks --------------------------------------------------
#
# A bare pane target that is not a pane falls back to a window, then to a
# session, using the current session (alpha).
TERMO_PANE=$pane TMUX_PANE=$pane check "editor" "0" '#{window_index}'	# bare window name
...
out=$($TERMO new-window -d -t 'alpha:1.%0' 2>&1)
