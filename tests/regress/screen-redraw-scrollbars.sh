...
new_scene() {
	$TERMO2 neww -d "sh -c 'i=0; while [ \$i -lt 11 ]; do printf \"SB%02d abcdefghij\n\" \$i; i=\$((i + 1)); done; exec sleep 100'" || exit 1
	$TERMO2 selectw -t:\$ || exit 1
	$TERMO2 resizew -x40 -y12 || exit 1
}

$TERMO kill-server 2>/dev/null
$TERMO2 kill-server 2>/dev/null

$TERMO2 new -d -x40 -y12 "sh -c 'i=0; while [ \$i -lt 11 ]; do printf \"SB%02d abcdefghij\n\" \$i; i=\$((i + 1)); done; exec sleep 100'" || exit 1
$TERMO2 set -g status off || exit 1
$TERMO2 set -g window-size manual || exit 1
$TERMO2 set -g pane-scrollbars on || exit 1
...
new_scene
$TERMO2 setw pane-scrollbars-position right || exit 1
$TERMO2 setw pane-scrollbars-style "bg=black,fg=white,width=2,pad=0" || exit 1
...
new_scene
$TERMO2 setw pane-scrollbars-position right || exit 1
$TERMO2 setw pane-scrollbars-style "bg=black,fg=white,width=1,pad=1" || exit 1
...
$TERMO2 setw pane-scrollbars-position right || exit 1
$TERMO2 setw pane-scrollbars-style "bg=black,fg=white,width=1,pad=0" || exit 1
$TERMO2 new-pane -x20 -y6 -X8 -Y3 "sh -c 'i=0; while [ \$i -lt 5 ]; do printf \"FLOAT%02d abcdef\n\" \$i; i=\$((i + 1)); done; exec sleep 100'" || exit 1
...
$TERMO2 setw pane-scrollbars-position right || exit 1
$TERMO2 setw pane-scrollbars-style "bg=black,fg=white,width=1,pad=0" || exit 1
$TERMO2 splitw -h "sh -c 'i=0; while [ \$i -lt 11 ]; do printf \"SBR%02d abcdefghij\n\" \$i; i=\$((i + 1)); done; exec sleep 100'" || exit 1
...
$TERMO2 setw pane-scrollbars-position left || exit 1
$TERMO2 setw pane-scrollbars-style "bg=black,fg=white,width=1,pad=0" || exit 1
$TERMO2 splitw -h "sh -c 'i=0; while [ \$i -lt 11 ]; do printf \"SBL%02d abcdefghij\n\" \$i; i=\$((i + 1)); done; exec sleep 100'" || exit 1
$TERMO2 selectp -t0 || exit 1
...
$TERMO2 neww -d "sh -c 'seq 40; exec sleep 100'" || exit 1
$TERMO2 selectw -t:\$ || exit 1
$TERMO2 resizew -x40 -y12 || exit 1
$TERMO2 setw pane-scrollbars-position right || exit 1
$TERMO2 setw pane-scrollbars-style "bg=black,fg=white,width=1,pad=0" || exit 1
