#!/bin/sh

# Exercise tty_draw_line through a real client. An inner termo is attached
# inside an outer termo pane; the outer pane is then captured to inspect what
# the inner client actually drew.

PATH=/bin:/usr/bin
TERM=screen
LC_ALL=C.UTF-8
export TERM LC_ALL

[ -z "$TEST_TERMO" ] && TEST_TERMO="${TEST_TMUX:-$(readlink -f ../../build/termo 2>/dev/null || readlink -f ../termo 2>/dev/null || which termo)}"
TEST_TMUX="$TEST_TERMO"
TERMO="$TEST_TERMO -LtestA$$ -f/dev/null"
TMUX="$TERMO"
TERMO2="$TEST_TERMO -LtestB$$ -f/dev/null"
TMUX2="$TERMO2"

fail() {
	echo "$*" >&2
	exit 1
}

capture() {
	$TERMO capturep -pS0 -E- >$TMP || exit 1
}

capturee() {
	$TERMO capturep -peS0 -E- >$TMP || exit 1
}

capturen() {
	$TERMO capturep -pNS0 -E- >$TMP || exit 1
}

captureen() {
	$TERMO capturep -peNS0 -E- >$TMP || exit 1
}

timed() {
	if command -v timeout >/dev/null 2>&1; then
		timeout 5 "$@"
	else
		"$@"
	fi
}

capture_timed() {
	timed $TERMO capturep -pS0 -E- >$TMP || exit 1
}

check_line() {
	line=$1
	want=$2
	got=$(sed -n "$line"p $TMP)
	[ "$got" = "$want" ] || fail "line $line: expected '$want', got '$got'"
}

check_grep() {
	pattern=$1
	grep -q "$pattern" $TMP || fail "missing pattern: $pattern"
}

$TERMO kill-server 2>/dev/null
$TERMO2 kill-server 2>/dev/null

TMP=$(mktemp)
trap "rm -f $TMP; $TERMO kill-server 2>/dev/null; $TERMO2 kill-server 2>/dev/null" 0 1 15

$TERMO2 -f/dev/null new -d -x20 -y6 -s test \
	"printf 'abcdefghijklmnopqrst'; exec sleep 100" || exit 1
$TERMO2 set -g status off || exit 1
$TERMO2 set -g mode-style "fg=white,bg=red" || exit 1
$TERMO2 setw -g mode-keys vi || exit 1
$TERMO2 neww -d \
	"printf '\033[31;44;1mRED\033[0m\tTAIL\nu:e\314\201:\347\225\214:\360\237\207\272\360\237\207\270:Z\nAAA      BBB'; exec sleep 100" || exit 1
$TERMO2 neww -d \
	"printf '12345678\347\225\214Z'; exec sleep 100" || exit 1
$TERMO2 neww -d \
	"printf 'wrap-ABCDEFGHIJKLMNOZ'; exec sleep 100" || exit 1
$TERMO2 neww -d \
	"printf 'AA    BB'; exec sleep 100" || exit 1
$TERMO2 neww -d \
	"printf 'XYZ'; exec sleep 100" || exit 1
$TERMO2 neww -d \
	"printf '\347\225\214\347\225\214\347\225\214\347\225\214\347\225\214'; exec sleep 100" || exit 1
$TERMO2 neww -d \
	"printf '123456789Z'; exec sleep 100" || exit 1
$TERMO2 neww -d \
	"printf 'abcdef\347\225\214GHIJKLMNOPQRSTUVWXYZ'; printf '\033[H'; exec sleep 100" || exit 1
$TERMO2 neww -d \
	"printf 'abcdefghijklmnopqrst\r\033[KXYZ\nnext'; exec sleep 100" || exit 1
$TERMO2 neww -d \
	"printf 'ab\tcdefghijklmnopqrstuvwxyz'; printf '\033[H'; exec sleep 100" || exit 1
$TERMO2 neww -d \
	"printf '\033(0x\033(B'; exec sleep 100" || exit 1
$TERMO2 neww -d \
	"awk 'BEGIN { for (i = 0; i < 1100; i++) printf \"a\" }'; exec sleep 100" || exit 1
$TERMO2 neww -d \
	"printf 'u\314\245\314\245\314\245\314\245\314\245\314\245\314\245\314\245\314\245\314\245\314\245\314\245\314\245\314\245\314\245\314\245'; exec sleep 100" || exit 1
$TERMO2 selectw -t:0 || exit 1

$TERMO -f/dev/null new -d -x20 -y6 || exit 1
$TERMO set -g status off || exit 1
$TERMO send -l "$TERMO2 attach" || exit 1
$TERMO send Enter || exit 1
sleep 1
CLIENT=$($TERMO2 list-clients -F '#{client_name}' | head -1)
[ -n "$CLIENT" ] || fail "no inner client"

# A variation selector which widens the previous character must use window
# coordinates when checking visibility in a pane with a nonzero x offset.
$TERMO2 set -s variation-selector-always-wide on || exit 1
WINDOW=$($TERMO2 neww -dPF '#{window_id}' "exec sleep 100") || exit 1
PANE=$($TERMO2 splitw -dhPF '#{pane_id}' -t "$WINDOW" \
	"exec sleep 100") || exit 1
$TERMO2 selectw -t "$WINDOW" || exit 1
$TERMO2 respawnp -k -t "$PANE" \
	"printf '\nA\342\234\217\357\270\217B'; exec sleep 100" || exit 1
sleep 1
$TERMO2 capturep -p -t "$PANE" >$TMP || exit 1
EXPECTED=$(printf 'A\342\234\217\357\270\217B')
check_line 2 "$EXPECTED"
$TERMO2 killw -t "$WINDOW" || exit 1

# Long line, then short line: default cells after cellsize must clear stale text.
capture
check_line 1 "abcdefghijklmnopqrst"
$TERMO2 selectw -t:5 || exit 1
sleep 1
capture
check_line 1 "XYZ"
grep -q '^XYZdefghijklmnopqrst$' $TMP && fail "short redraw left stale tail"

# Styles, tabs, same runs, Unicode combining/wide/flag cells.
$TERMO2 selectw -t:1 || exit 1
sleep 1
capture
check_grep '^RED[	 ]*TAIL$'
check_grep '^u:e.*:.*:.*:Z$'
check_line 3 "AAA      BBB"
capturee
esc=$(printf '\033')
grep -q "$esc" $TMP || fail "styled redraw did not preserve attributes"

# Wide character clipping and padding after resize.
$TERMO2 selectw -t:2 || exit 1
$TERMO resizew -x10 -y6 || exit 1
sleep 1
$TERMO2 respawnp -k "printf '12345678\347\225\214Z'; exec sleep 100" || exit 1
sleep 1
capture
check_line 1 "12345678界"
$TERMO resizew -x9 -y6 || exit 1
sleep 1
capture
check_line 1 "12345678"
grep -q '^12345678Z$' $TMP && fail "wide clipping left stale cell"

# Repeated wide characters at the right edge should not leave orphan padding.
$TERMO resizew -x9 -y6 || exit 1
$TERMO2 selectw -t:6 || exit 1
sleep 1
$TERMO2 respawnp -k "printf '\347\225\214\347\225\214\347\225\214\347\225\214\347\225\214'; exec sleep 100" || exit 1
sleep 1
capture
check_line 1 "界界界界"

# Tabs should clear stale cells as an empty run.
$TERMO resizew -x10 -y6 || exit 1
$TERMO2 selectw -t:7 || exit 1
sleep 1
capture
check_line 1 "123456789Z"
$TERMO2 respawnp -k "printf '123456789\t'; exec sleep 100" || exit 1
sleep 1
capture
check_line 1 "123456789"
grep -q '^123456789.*Z$' $TMP && fail "tab clipping left stale cell"

# Tabs clipped at both ends, at the right, and at the left. This is like
# drawing spans over the middle of a tab when an overlay or viewport clips the
# pane line.
$TERMO resizew -x4 -y6 || exit 1
$TERMO2 selectw -t:10 || exit 1
$TERMO2 resizew -t:10 -x26 -y6 || exit 1
$TERMO2 respawnp -k "printf 'ab\tcdefghijklmnopqrstuvwxyz'; printf '\033[H'; exec sleep 100" || exit 1
$TERMO2 refresh -t"$CLIENT" -c || exit 1
sleep 1
capturen
check_line 1 "ab  "
$TERMO2 refresh -t"$CLIENT" -R 2 || exit 1
sleep 1
capturen
check_line 1 "    "
$TERMO2 refresh -t"$CLIENT" -R 4 || exit 1
sleep 1
capturen
check_line 1 "  cd"
$TERMO2 refresh -t"$CLIENT" -R 2 || exit 1
sleep 1
capturen
check_line 1 "cdef"
$TERMO2 refresh -t"$CLIENT" -L 8 || exit 1

# Horizontal clipping that starts in or near a wide character should not draw
# partial padding or stale cells.
$TERMO resizew -x20 -y6 || exit 1
$TERMO2 selectw -t:8 || exit 1
$TERMO2 resizew -t:8 -x26 -y6 || exit 1
$TERMO2 respawnp -k "printf 'abcdef\347\225\214GHIJKLMNOPQRSTUVWXYZ'; printf '\033[H'; exec sleep 100" || exit 1
$TERMO2 refresh -t"$CLIENT" -c || exit 1
sleep 1
$TERMO2 refresh -t"$CLIENT" -R 4 || exit 1
sleep 1
capture
check_line 1 "ef界GHIJKLMNOPQRSTUV"
$TERMO2 refresh -t"$CLIENT" -R 1 || exit 1
sleep 1
capture
check_line 1 "f界GHIJKLMNOPQRSTUVW"
$TERMO2 refresh -t"$CLIENT" -L 5 || exit 1

# Wrapped line redraw.
$TERMO resizew -x20 -y6 || exit 1
$TERMO2 selectw -t:3 || exit 1
sleep 1
capture
check_line 1 "wrap-ABCDEFGHIJKLMNO"
check_line 2 "Z"

# Selection over spaces should still paint attributes for otherwise empty cells.
$TERMO2 selectw -t:4 || exit 1
sleep 1
$TERMO2 copy-mode -H || exit 1
$TERMO2 send -X start-of-line || exit 1
$TERMO2 send -X -N 2 cursor-right || exit 1
$TERMO2 send -X begin-selection || exit 1
$TERMO2 send -X -N 3 cursor-right || exit 1
sleep 1
capture
check_line 1 "AA    BB"
capturee
sed -n 1p $TMP | grep -q "$esc" || fail "selected spaces did not draw attributes"

# Selection on a short line should still draw attributes correctly. This line
# was previously expanded, then cleared and rewritten shorter, so cellsize
# remains larger than the visible text.
$TERMO2 selectw -t:9 || exit 1
sleep 1
$TERMO2 copy-mode -H || exit 1
$TERMO2 send -X history-top || exit 1
$TERMO2 send -X start-of-line || exit 1
$TERMO2 send -X begin-selection || exit 1
$TERMO2 send -X cursor-down || exit 1
sleep 1
capture
check_line 1 "XYZ"
check_line 2 "next"
captureen
sed -n 1p $TMP | grep -q "$esc" || \
	fail "selected short-line tail did not draw attributes"

# ACS/charset cells should be redrawn correctly.
$TERMO resizew -x20 -y6 || exit 1
$TERMO2 selectw -t:11 || exit 1
sleep 1
capture
check_line 1 "│"

# A long run with the same attributes should flush the internal draw buffer.
$TERMO resizew -x1100 -y6 || exit 1
$TERMO2 resizew -t:12 -x1100 -y6 || exit 1
$TERMO2 selectw -t:12 || exit 1
sleep 1
capture
len=$(sed -n 1p $TMP | wc -c)
[ "$len" -ge 1100 ] || fail "long same-style line was truncated"

# Too many combining marks on one base character must not leave a standalone
# width-zero cell that can make tty_draw_line loop forever on redraw.
$TERMO resizew -x20 -y6 || exit 1
timed $TERMO2 selectw -t:13 || fail "zero-width overflow select hung"
sleep 1
capture_timed
check_grep '^u'

exit 0
