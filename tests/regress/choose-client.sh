...
$TERMO choose-client -t aaa:0 -F 'C2=#{client_session}' \
...
$TERMO choose-client -t aaa:0 -F 'C3=#{client_session}' || exit 1
wait_count 'C3=' 2
out=$CAPTURED
printf '%s\n' "$out" | grep -F -q 'C3=aaa' || \
...
$TERMO choose-client -t aaa:0 -F 'C4=#{client_session}' \
...
$TERMO choose-client -t aaa:0 -F 'G1=#{client_session}' \
	-f '#{==:#{client_session},bbb}' || exit 1
wait_for 'G1=bbb'
...
wait_mode aaa:0 0
wait_gone 'G1='
[ "$($TERMO list-clients -F '#{client_session}')" = "aaa" ] || \
