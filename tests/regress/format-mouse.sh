...
# we send it.
$TERMO new-session -d -s cov -x 80 -y 24 'cat' || exit 1
$TERMO set -g mouse on
sleep 1
$TERMO send-keys -t cov:0.0 'alpha beta gamma' Enter
sleep 1

# Record every pane mouse variable when the pane is clicked.
$TERMO bind -n MouseDown1Pane run-shell \
    "$TERMO set -g @m 'x=#{mouse_x} y=#{mouse_y} word=#{mouse_word} line=#{mouse_line} pane=#{mouse_pane} hl=[#{mouse_hyperlink}]'"

# Attach a real client inside an outer termo pane.  Clicks all target the first
# row, which lines up with the inner client regardless of the outer status line.
$TERMO2 new-session -d -x 80 -y 24 "$TERMO attach -t cov" || exit 1
sleep 1
...
$TERMO bind -T copy-mode MouseDown1Pane run-shell \
    "$TERMO set -g @cm 'x=#{mouse_x} word=#{mouse_word} line=#{mouse_line}'"
$TERMO copy-mode -t cov:0.0
sleep 1
click 8 1
CM=$($TERMO show -gv @cm 2>/dev/null)
case "$CM" in
*"word=beta"*) ;;
*) fail "Unexpected copy-mode mouse_word in: $CM" ;;
esac
$TERMO send-keys -t cov:0.0 -X cancel
sleep 1
...
EOF
chmod +x "$LINKSH"
$TERMO neww -t cov: -n link "$LINKSH"
sleep 1
$TERMO select-window -t cov:link
sleep 1
click 3 1
M=$($TERMO show -gv @m 2>/dev/null)
rm -f "$LINKSH"
