...
# rich context - a real attached client (from a nested termo), a control-mode
...
test_var()
{
	...
	out=$($TERMO display-message "$@" -p "#{$name}")
	if [ "$out" != "$exp" ]; then
...
$TERMO new-session -d -s cov -x 80 -y 24 -n win0 'cat' || exit 1
$TERMO set -g automatic-rename off
$TERMO set -g monitor-bell on
$TERMO set -g monitor-activity on
$TERMO split-window -t cov:win0 -d 'cat' || exit 1
$TERMO new-window -d -t cov:1 -n win1 'cat' || exit 1
$TERMO set -g @opt 'optionvalue' || exit 1
$TERMO set-buffer -b buf0 'somebuffer' || exit 1

# A second session grouped with cov, so the session_group_* variables have real
# data to report.
$TERMO new-session -d -s cov2 -t cov || exit 1

sleep 1
$TERMO send-keys -t cov:win0.0 'some pane content' Enter
...
$TERMO send-keys -t cov:win1.0 C-g
sleep 1
...
HOLD=$!
$TERMO -C attach -t cov <"$FIFO" >/dev/null 2>&1 &
CC=$!

# Attach a real client too: an inner termo running inside a pane of the second
...
$TERMO2 new-session -d -x 90 -y 30 "$TERMO attach -t cov" || exit 1
sleep 1

# The real (terminal) client, identified by not being in control mode.
RC=$($TERMO list-clients -F '#{client_control_mode} #{client_name}' |
    awk '$1==0 { print $2; exit }')
# The control client.
CLIENT=$($TERMO list-clients -F '#{client_control_mode} #{client_name}' |
...
test_var pid "$($TERMO display-message -p '#{pid}')" -t "$TGT"
...
test_var socket_path "$($TERMO display-message -p '#{socket_path}')" -c "$CLIENT"
# The real client has a terminal, so termcap/feature/environ queries work.
test_var "I/e:TERM" "$($TERMO display-message -c "$RC" -p '#{client_termname}')" \
    -c "$RC"
# Termcap and feature queries against a real terminal return a boolean.
case "$($TERMO display-message -c "$RC" -p '#{I/c:colors}')" in
0|1) ;;
*) fail "Unexpected #{I/c:colors} for real client." ;;
esac
case "$($TERMO display-message -c "$RC" -p '#{I/f:256}')" in
0|1) ;;
...
[ -n "$($TERMO display-message -p '#{t/p:start_time}')" ] ||
    fail "Empty #{t/p:start_time}."
[ -n "$($TERMO display-message -p '#{t/r:start_time}')" ] ||
...
$TERMO new-session -d -s quot -x 80 -y 24 -- sh -c 'sleep 100' arg0 \
    "it's a 'test'" 'two words' '' 'new
line' || fail "Failed to create quoting test session."
LIST=$($TERMO -u display-message -t 'quot:0.0' -p '#{pane_start_command_list}')
eval "set -- $LIST"
...
$TERMO new-window -d -t 'quot:' || fail "Failed to create shell window."
