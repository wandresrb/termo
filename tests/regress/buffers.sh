...
TMP=$(mktemp)
TMP2=$(mktemp)
trap 'rm -f "$TMP" "$TMP2" "$TMP".*; $TERMO kill-server 2>/dev/null' 0 1 15
...
out=$($TERMO list-buffers -f '#{==:#{buffer_name},other}' -F '#{buffer_name}')
if [ "$out" != "other" ]; then
...
out=$(echo $($TERMO list-buffers -F '#{buffer_sample}'))
...
paste_line()
{
	$TERMO kill-window -t B:9 2>/dev/null
	check_ok new-window -d -t B:9 'stty raw -echo && exec cat -v'
	i=0
	while [ "$($TERMO display-message -p -t B:9.0 \
	...
	i=0
	while out=$($TERMO capture-pane -p -t B:9.0 | sed -n 1p) && \
	    [ "$out" != "$3" ]; do
		i=$((i + 1))
		if [ $i -gt 50 ]; then
...
out=$($TERMO save-buffer -b sb /nonexistent/dir/file 2>&1)
if [ $? -eq 0 ]; then
...
reader=$!
exec 3>"$TMP.fifo"
out=$($TERMO save-buffer -b write-error "$TMP.fifo" 3>&- 2>&1)
status=$?
exec 3>&-
wait "$reader"
if [ $status -eq 0 ]; then
...
out=$($TERMO save-buffer -b sb -)
if [ "$out" != "data" ]; then
	echo "save-buffer - wrong: '$out'"
	exit 1
fi
check_ok delete-buffer -b sb
printf 'from stdin' | $TERMO load-buffer -b stdinbuf -
