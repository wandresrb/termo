...
$TERMO capture-pane -peH -t hyperlink: -S 0 -E - >/dev/null || exit 1
...
$TERMO respawn-pane -k -t title-stack: \
...
sleep 0.1
$TERMO new-session -d -x 20 -y 3 -s osc52 "sleep 2" || exit 1
$TERMO set-option -s set-clipboard on || exit 1
$TERMO respawn-pane -k -t osc52: \
    "printf '\033]52;c;SGVsbG8=\007'; sleep 2" || exit 1
sleep 0.3
$TERMO save-buffer -b buffer0 - >"$TMP"
printf "Hello" >"$EXP"
