...
TMUX="$TERMO"
OUT="$TEST_TERMO -LtestB$$ -f/dev/null"
...
wait_mode()
{
	...
	i=0
	while [ "$i" -lt 50 ]; do
		got=$($TERMO display-message -p -t cm:0 '#{pane_in_mode}' \
		    2>/dev/null)
		[ "$got" = "$want" ] && return 0
		sleep 0.2
		i=$((i + 1))
...
{
	cleanup_servers
	$TERMO new-session -d -s cm -n main -x 80 -y 24 'cat' ||
		fail "new-session failed"
	$OUT new-session -d -s out -x 80 -y 24 "$TERMO attach -t cm" ||
...
{
	if [ "$#" -eq 0 ]; then
		$TERMO customize-mode -t cm:0 -F "$FORMAT" ||
			fail "customize-mode failed"
	else
		$TERMO customize-mode -t cm:0 -F "$FORMAT" -f "$1" ||
			fail "customize-mode failed"
	fi
	wait_mode 1
	settle
...
send()
{
	$TERMO send-keys -t cm:0 "$@" || fail "send-keys failed: $*"
}

send_literal()
{
	$TERMO send-keys -t cm:0 -l "$1" || fail "send literal failed: $1"
}
...
test_user_option()
{
	...
	send Enter
	settle
	assert_equals "$($TERMO show-options -sqv @cm_add)" "added" \
	...
	assert_equals "$($TERMO show-options -sqv @cm_add)" "changed" \
		"set user option"

	send u y
	settle
	assert_empty "$($TERMO show-options -sqv @cm_add)" "unset user option"
	send q
	wait_mode 0
}
...
test_reset_window_pane_option()
{
	...
	$TERMO set-option -g window-style fg=red ||
		fail "set global window-style failed"
	$TERMO set-window-option -t cm:0 window-style fg=green ||
		fail "set window window-style failed"
	$TERMO set-option -p -t cm:0.0 window-style fg=blue ||
	...
	assert_equals "$($TERMO show-option -gqv window-style)" "default" \
		"reset global window-style"
	assert_empty "$($TERMO show-options -wqv -t cm:0 window-style)" \
		"reset window window-style"
	assert_empty "$($TERMO show-option -p -t cm:0.0 -qv window-style)" \
...
test_environment()
{
	...
	send Enter
	settle
	assert_equals "$($TERMO show-environment -t cm CM_ENV)" \
	...
	assert_equals "$($TERMO show-environment -g CM_ENV)" \
		"CM_ENV=global" "set global environment"

	send u y
	settle
	$TERMO show-environment -t cm CM_ENV >/dev/null 2>&1 &&
...
test_user_hook()
{
	...
	send Enter
	settle
	assert_equals "$($TERMO show-option -qv @cm_hook)" \
		"display-message hook" "add user hook"
	$TERMO set-hook -E @cm_hook || fail "fire user hook failed"
	...
	send u y
	settle
	assert_empty "$($TERMO show-option -qv @cm_hook)" "unset user hook"
	send q
	wait_mode 0
}
...
test_hook_array()
{
	...
	$TERMO set-hook -g 'after-new-session[0]' 'display-message old' ||
		fail "set hook array failed"
	$TERMO set-hook -g -R after-new-session || fail "fire hook array failed"
	...
	settle
	replace_prompt "1"
	hooks=$($TERMO show-hooks -g after-new-session)
...
test_changed_only()
{
	...
	$TERMO set-option -s @cm_changed yes ||
		fail "set changed server option failed"
	$TERMO bind-key -T prefix User1 display-message changed-only ||
...
test_reset_key()
{
	...
	orig=$($TERMO list-keys -T prefix -F '#{key_command}' c) ||
		fail "default c binding not found"
	$TERMO bind-key -T prefix c display-message changed ||
	...
	send Right j d y
	settle
	after=$($TERMO list-keys -T prefix -F '#{key_command}' c) ||
...
test_tagged_unset()
{
	...
	$TERMO set-option -s @cm_tag_a one || fail "set tag option a failed"
	$TERMO set-option -s @cm_tag_b two || fail "set tag option b failed"

	open_customize '#{m/r:^@cm_tag_,#{option_name}}'
	send Right j t j t U y
	settle
	assert_empty "$($TERMO show-options -sqv @cm_tag_a)" \
		"tagged unset option a"
	assert_empty "$($TERMO show-options -sqv @cm_tag_b)" \
...
test_key_binding()
{
	...
	send Enter
	settle
	keys=$($TERMO list-keys -T prefix User0 2>/dev/null) ||
	...
	note=$($TERMO list-keys -T prefix -F '#{key_note}' User0)
	...
	send u y
	settle
	$TERMO list-keys -T prefix User0 >/dev/null 2>&1 &&
...
test_editor()
{
	...
	$TERMO set-option -g editor "$editor" || fail "set editor failed"
	$TERMO set-option -g @cm_edit old || fail "set edit option failed"
	...
	i=0
	while [ "$i" -lt 50 ]; do
		value=$($TERMO show-option -gqv @cm_edit)
		[ "$value" = "edited" ] && break
		sleep 0.2
		i=$((i + 1))
	done
	...
	$TERMO bind-key -T prefix User2 display-message old ||
	...
	i=0
	while [ "$i" -lt 50 ]; do
		value=$($TERMO list-keys -T prefix -F '#{key_command}' User2)
	...
	i=0
	while [ "$i" -lt 50 ]; do
		value=$($TERMO list-keys -T prefix -F '#{key_note}' User2)
	...
	$TERMO set-environment -t cm CM_EDIT_ENV old ||
	...
	i=0
	while [ "$i" -lt 50 ]; do
		value=$($TERMO show-environment -t cm CM_EDIT_ENV)
...
test_stale_current_items()
{
	...
	$TERMO set-option -s @cm_stale_option old ||
		fail "set stale option failed"
	open_customize '#{==:#{option_name},@cm_stale_option}'
	send Right j
	settle
	$TERMO set-option -su @cm_stale_option ||
	...
	$TERMO set-environment -t cm CM_STALE_ENV old ||
		fail "set stale environment failed"
	open_customize '#{==:#{environment_name},CM_STALE_ENV}'
	repeat_key j 6
	send Right j
	settle
	$TERMO set-environment -t cm -u CM_STALE_ENV ||
	...
	$TERMO bind-key -T prefix User3 display-message stale ||
		fail "set stale key failed"
	open_customize '#{==:#{key},User3}'
	repeat_key j 10
	send Right j
	settle
	$TERMO unbind-key -T prefix User3 ||
...
test_prompt_invalidation()
{
	...
	$TERMO set-hook -g 'after-new-session[1]' 'display-message taken' ||
...
test_invalid_prompts()
{
	...
	send Enter
	settle
	assert_empty "$($TERMO show-options -sqv bad)" "invalid user option name"
	...
	$TERMO bind-key -T prefix User6 display-message valid ||
	...
	keys=$($TERMO list-keys -T prefix -F '#{key_command}' User6)
