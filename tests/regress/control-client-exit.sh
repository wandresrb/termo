...
$TERMO -f/dev/null new -d -x 80 -y 24 -s rt 'cat /dev/zero | tr "\000" x' || exit 1
$TERMO setw -g automatic-rename off || exit 1
...
( sleep 30 ) | $TERMO -f/dev/null -C attach -t rt >"$FIFO" 2>&1 &
exec 8<"$FIFO"

n=0
while [ $n -lt 50 ]; do
	$TERMO lsc -F '#{client_name}' 2>/dev/null | grep -q . && break
	sleep 0.1
	n=$((n + 1))
done
$TERMO lsc -F '#{client_name}' 2>/dev/null | grep -q . ||
...
i=0
while [ $i -lt 5 ]; do
	$TERMO rename-window -t rt:0 "$MARKER$i"
	i=$((i + 1))
done
sleep 0.5
