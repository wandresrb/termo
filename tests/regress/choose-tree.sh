...
# in environ-update.sh - a second server provides a client: an inner "termo
...
exit_mode()
{
	$TERMO send-keys -t aaa:0 "$@" || fail "send-keys $* failed"
	wait_mode aaa:0 0
}
...
$TERMO new-session -d -s zzz -x 80 -y 24 'cat' || exit 1
$TERMO split-window -t zzz:0 'cat' || exit 1
$TERMO new-window -t zzz 'cat' || exit 1
$TERMO new-session -d -s aaa -x 80 -y 24 'cat' || exit 1

$TERMO2 new-session -d -s out -x 80 -y 24 "$TERMO attach -t aaa" || exit 1
...
# (session, window, pane).
$TERMO choose-tree -t aaa:0 -F 'F1' -f '#{==:#{session_name},aaa}' || exit 1
wait_count ': F1' 3
out=$CAPTURED
printf '%s\n' "$out" | grep -F -q 'aaa: F1' || \
...
# disappear.
$TERMO choose-tree -t aaa:0 -F 'F2' -f '#{==:#{session_name},zzz}' || exit 1
wait_count ': F2' 6
out=$CAPTURED
printf '%s\n' "$out" | grep -F -q 'zzz: F2' || \
...
$TERMO choose-tree -t aaa:0 -F 'F3' -f '#{==:#{pane_index},1}' || exit 1
wait_count ': F3' 3
out=$CAPTURED
printf '%s\n' "$out" | grep -F -q 'zzz: F3' || \
...
$TERMO choose-tree -t aaa:0 -F 'F4' -f '#{==:#{session_name},nosuch}' || exit 1
wait_count ': F4' 9
out=$CAPTURED
printf '%s\n' "$out" | grep -F -q 'aaa: F4' || \
...
$TERMO choose-tree -h -t aaa:0 -F 'F5' -f '#{==:#{session_name},aaa}' || \
	exit 1
wait_count ': F5' 2
...
$TERMO choose-tree -t aaa:0 -F 'F6' -O index || exit 1
wait_count ': F6' 9
...
$TERMO choose-tree -t aaa:0 -F 'F7' -O name || exit 1
wait_count ': F7' 9
...
$TERMO choose-tree -t aaa:0 -F 'F8' -O name -r || exit 1
wait_count ': F8' 9
...
# l expands it again.
$TERMO choose-tree -t aaa:0 -F 'G1' -O index || exit 1
wait_count ': G1' 9
$TERMO send-keys -t aaa:0 g h || fail "send-keys collapse failed"
wait_count ': G1' 4
$TERMO send-keys -t aaa:0 l || fail "send-keys expand failed"
wait_count ': G1' 9
exit_mode q

# --- filter entered at the prompt with f, cleared with c ----------------------
$TERMO choose-tree -t aaa:0 -F 'G2' -O index || exit 1
wait_count ': G2' 9
$TERMO send-keys -t aaa:0 f || fail "send-keys f failed"
$TERMO send-keys -t aaa:0 -l '#{==:#{session_name},aaa}' || \
	fail "send-keys filter failed"
$TERMO send-keys -t aaa:0 Enter || fail "send-keys Enter failed"
wait_count ': G2' 3
out=$CAPTURED
printf '%s\n' "$out" | grep -F -q 'aaa: G2' || \
...
$TERMO send-keys -t aaa:0 c || fail "send-keys c failed"
wait_count ': G2' 9
exit_mode q
...
$TERMO choose-tree -t aaa:0 -F 'G3' -O index || exit 1
wait_count ': G3' 9
$TERMO send-keys -t aaa:0 g Enter || fail "send-keys Enter failed"
i=0
while [ "$i" -lt 50 ]; do
	[ "$($TERMO list-clients -F '#{client_session}')" = "zzz" ] && break
	sleep 0.2
	i=$((i + 1))
done
[ "$i" -lt 50 ] || fail "client did not switch to zzz"
$TERMO switch-client -c "$($TERMO list-clients -F '#{client_name}')" -t aaa || \
	exit 1
wait_mode aaa:0 0
...
$TERMO choose-tree -t aaa:0 -F 'G4' -O index || exit 1
wait_count ': G4' 9
$TERMO send-keys -t aaa:0 g j j j j x || fail "send-keys x failed"
wait_for 'Kill window 1'
$TERMO send-keys -t aaa:0 y || fail "send-keys y failed"
wait_count ': G4' 7
[ "$($TERMO list-windows -t zzz -F x | grep -c x)" -eq 1 ] || \
