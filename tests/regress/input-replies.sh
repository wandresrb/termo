...
$TERMO new-session -d -x 80 -y 24 -s replies \; \
    set-window-option -t replies:0 remain-on-exit on || exit 1
$TERMO set-option -s set-clipboard on || exit 1
$TERMO set-option -s get-clipboard buffer || exit 1
printf Hello | $TERMO load-buffer -
sleep 0.5
...
query()
{
	...
	$TERMO respawn-window -k -t replies:0 \
