...
#
# Each scene is rendered in an inner termo attached inside an outer termo pane. The
...
new_scene() {
	$TERMO2 neww -d "sh -c 'i=0; while [ \$i -lt 13 ]; do printf \"BASE%02d abcdefghij\n\" \$i; i=\$((i + 1)); done; exec sleep 100'" || exit 1
...
$TERMO2 new -d -x40 -y14 "sh -c 'exec sleep 100'" || exit 1
...
$TERMO2 resize-pane -t0 -L 6 || exit 1
...
$TERMO2 selectp -t0 || exit 1
$TERMO2 splitw -h "$C" || exit 1
$TERMO2 selectp -t:.0 -T AAA || exit 1
$TERMO2 selectp -t:.1 -T BBB || exit 1
$TERMO2 selectp -t:.2 -T CCC || exit 1
$TERMO2 selectp -t:.0 || exit 1
...
$TERMO2 setw pane-border-status top || exit 1
$TERMO2 splitw -h "$C" || exit 1
$TERMO2 selectp -t:.0 -T LEFT || exit 1
$TERMO2 selectp -t:.1 -T RIGHT || exit 1
$TERMO2 selectp -t:.0 || exit 1
compare cache-swap-before

$TERMO2 swap-pane -d -s:.0 -t:.1 || exit 1
$TERMO2 selectp -t:.0 || exit 1
...
$TERMO2 neww -d "sh -c 'i=0; while [ \$i -lt 13 ]; do printf \"WINA%02d abcdefghij\n\" \$i; i=\$((i + 1)); done; exec sleep 100'" || exit 1
$TERMO2 selectw -t:\$ || exit 1
$TERMO2 resizew -x40 -y14 || exit 1
A=$($TERMO2 display -p '#{window_id}') || exit 1

# Window B: a top/bottom split (a horizontal border).
$TERMO2 neww -d "sh -c 'i=0; while [ \$i -lt 13 ]; do printf \"WINB%02d abcdefghij\n\" \$i; i=\$((i + 1)); done; exec sleep 100'" || exit 1
$TERMO2 selectw -t:\$ || exit 1
$TERMO2 resizew -x40 -y14 || exit 1
$TERMO2 splitw -v "$C" || exit 1
B=$($TERMO2 display -p '#{window_id}') || exit 1

$TERMO2 selectw -t$A || exit 1
compare cache-window-a
$TERMO2 selectw -t$B || exit 1
...
$TERMO2 neww -d "sh -c 'i=0; while [ \$i -lt 13 ]; do printf \"SIZE%02d abcdefghij\n\" \$i; i=\$((i + 1)); done; exec sleep 100'" || exit 1
$TERMO2 selectw -t:\$ || exit 1
$TERMO2 splitw -v "$C" || exit 1
compare cache-resizeclient-before

$TERMO resizew -x30 -y10 || exit 1
