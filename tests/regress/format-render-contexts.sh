...
capture()
{
	out=$(
		if command -v timeout >/dev/null 2>&1; then
			timeout 10 $TERMO2 capture-pane -p -t out:0 2>/dev/null
		else
...
capture_esc()
{
	out=$(
		if command -v timeout >/dev/null 2>&1; then
			timeout 10 $TERMO2 capture-pane -pe -t out:0 \
				2>/dev/null
		else
			$TERMO2 capture-pane -pe -t out:0 2>/dev/null
		fi
	)
	rc=$?
...
assert_alive()
{
	termo_run "server alive ($1)" display-message -p alive >/dev/null
}
...
render_status_options()
{
	...
	termo_run "$label status-left" \
		set-option -g status-left "SL$label: $fmt" >/dev/null
	termo_run "$label clear status-right" \
		set-option -g status-right '' >/dev/null
	termo_run "$label status-format left" \
		set-option -g status-format[0] '#{E:status-left}' >/dev/null
	termo_run "$label refresh status-left" refresh-client -S >/dev/null
	wait_for "SL$label:"

	termo_run "$label clear status-left" \
		set-option -g status-left '' >/dev/null
	termo_run "$label status-right" \
		set-option -g status-right "SR$label: $fmt" >/dev/null
	termo_run "$label status-format right" \
		set-option -g status-format[0] \
		'#[align=right]#{E:status-right}' >/dev/null
	termo_run "$label refresh status-right" refresh-client -S >/dev/null
	wait_for "SR$label:"
...
render_status_format()
{
	...
	termo_run "$label status-format" \
		set-option -g status-format[0] "SF$label: $fmt" >/dev/null
	termo_run "$label refresh status-format" refresh-client -S >/dev/null
	wait_for "SF$label:"
...
render_message()
{
	...
	termo_run "$label display-message" \
...
render_choose_tree()
{
	...
	termo_run "$label choose-tree" \
		choose-tree -t fmt:0 -F "CT$label: $fmt" >/dev/null
	wait_for "CT$label:"
	termo_run "$label quit choose-tree" send-keys -t fmt:0 q >/dev/null
...
render_customize()
{
	...
	termo_run "$label customize-mode" \
		customize-mode -t fmt:0 -F "CM$label: $fmt" >/dev/null
	sleep 0.5
	out=$(capture)
	bounded "$label customize-mode capture" "$out"
	termo_run "$label quit customize-mode" send-keys -t fmt:0 q >/dev/null
...
termo_run "new inner session" \
	new-session -d -s fmt -x 100 -y 30 "exec sleep 1000" >/dev/null
termo_run "new inner window" \
	new-window -t fmt -n second "exec sleep 1000" >/dev/null
termo_run "select first window" select-window -t fmt:0 >/dev/null
termo_run "status interval" set-option -g status-interval 1 >/dev/null
termo_run "base status" set-option -g status on >/dev/null
termo_run "nested option" \
	set-option -g @nested 'NESTED-#{session_name}-#{window_index}' >/dev/null

run_cmd $TERMO2 new-session -d -s out -x 100 -y 30 "$TERMO attach -t fmt" \
	>/dev/null 2>&1 || fail "failed to start outer client"

i=0
while [ "$i" -lt 50 ]; do
	c=$(termo_run "wait clients" list-clients -F x | grep -c x)
	[ "$c" -eq 1 ] && break
	sleep 0.2
	i=$((i + 1))
done
