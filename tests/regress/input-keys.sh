...
sleep 1
$TERMO -f/dev/null new -x20 -y2 -d \; set -g escape-time 0 || exit 1
sleep 1
W=$($TERMO new-window -P -- sh -c 'stty raw -echo && cat -tv')
sleep 1
...
assert_key () {
	...
	$TERMO send-keys -t$W -R \; \
	      clear-history -t$W \; \
	      send-keys -t$W "$key" 'EOL' || exit 1

	actual_code=$($TERMO capturep -pt$W | \
			      head -1 | \
			      sed -e 's/EOL.*$//')
...
# Extended keys
$TERMO set -g extended-keys always
