...
check_fmt()
{
	out=$($TERMO display-message -p -t "$1" "$2" 2>&1)
	if [ "$out" != "$3" ]; then
...
check_panes()
{
	out=$(echo $($TERMO list-panes -t "$1" -F '#{pane_index}:#{pane_id}'))
	if [ "$out" != "$2" ]; then
...
p0=$($TERMO display-message -p -t P:0.0 '#{pane_id}')
...
p1=$($TERMO display-message -p -t P:0.1 '#{pane_id}')
...
p2=$($TERMO display-message -p -t P:0.1 '#{pane_id}')
...
p3=$($TERMO display-message -p -t P:0.0 '#{pane_id}')
...
p4=$($TERMO display-message -p -t P:0.2 '#{pane_id}')
check_ok split-window -v -t "$p4"
p5=$($TERMO display-message -p -t 'P:0' '#{pane_id}')
...
out=$($TERMO break-pane -d -P -F '#{window_index}:#{pane_id}' -n broken \
	-s "$p1" -t P:)
if [ "$out" != "1:$p1" ]; then
...
check_fmt 'P:0' '#{window_zoomed_flag}' '0'
if $TERMO has-session -t P:broken 2>/dev/null; then
	echo "Window 'broken' still exists after join-pane."
	exit 1
fi

# The joined pane is below the target (-v, no -b).
top=$($TERMO display-message -p -t "$p2" '#{pane_bottom}')
joined=$($TERMO display-message -p -t "$p1" '#{pane_top}')
...
l1=$($TERMO display-message -p -t "$p1" '#{pane_left}')
l2=$($TERMO display-message -p -t "$p2" '#{pane_left}')
if [ "$l1" -ge "$l2" ]; then
...
p6=$($TERMO display-message -p -t P:0.2 '#{pane_id}')
...
layout=$($TERMO display-message -p -t P:0 '#{window_layout}')
check_ok select-pane -t "$p0"
check_ok resize-pane -Z -t "$p0"
...
zk0=$($TERMO display-message -p -t P:12.0 '#{pane_id}')
zk1=$($TERMO split-window -d -P -F '#{pane_id}' -t P:12.0 'cat')
zk2=$($TERMO split-window -d -P -F '#{pane_id}' -t P:12.0 'cat')
...
ze0=$($TERMO display-message -p -t P:13.0 '#{pane_id}')
ze1=$($TERMO split-window -d -P -F '#{pane_id}' -t P:13.0 'cat')
ze2=$($TERMO split-window -d -P -F '#{pane_id}' -t P:13.0 'cat')
check_ok resize-pane -Z -t "$ze0"
check_ok send-keys -t "$ze1" C-d
i=0
while [ "$($TERMO display-message -p -t P:13 '#{window_panes}')" != 2 ]; do
	i=$((i + 1))
...
i=0
while [ "$($TERMO display-message -p -t P:13 '#{window_panes}')" != 1 ]; do
	i=$((i + 1))
...
zr0=$($TERMO display-message -p -t P:14.0 '#{pane_id}')
...
check_fmt "$zr0" \
	...
	'90x30:1:1'
check_ok resize-pane -Z -t "$zr0"
zrf=$($TERMO new-pane -dP -F '#{pane_id}' -t P:14 -x 20 -y 6 'cat')
zrf_size=$($TERMO display-message -p -t "$zrf" \
...
if $TERMO has-session -t P:goner 2>/dev/null; then
...
i=0
while [ "$($TERMO display-message -p -t P:8.0 '#{pane_dead}')" != "1" ]; do
	i=$((i + 1))
...
q0=$($TERMO display-message -p -t P:2.0 '#{pane_id}')
check_ok split-window -d -v -t "$q0"
q1=$($TERMO display-message -p -t P:2.1 '#{pane_id}')
check_ok split-window -d -h -t "$q1"
q2=$($TERMO display-message -p -t P:2.2 '#{pane_id}')
...
eid=$($TERMO split-window -d -P -F '#{pane_id}' -t P:2.0 '')
...
eid=$($TERMO split-window -d -P -F '#{pane_id}' -e GREETING=hello -t P:2.0 \
	'echo $GREETING; exec cat')
i=0
while out=$($TERMO capture-pane -p -t "$eid" | sed -n 1p) && \
    [ "$out" != "hello" ]; do
	i=$((i + 1))
...
check_ok split-window -d -v -t P:3.0
r0=$($TERMO display-message -p -t P:3.0 '#{pane_id}')
r1=$($TERMO display-message -p -t P:3.1 '#{pane_id}')
r2=$($TERMO display-message -p -t P:3.2 '#{pane_id}')
o0=$($TERMO display-message -p -t P:5.0 '#{pane_id}')
...
o1=$($TERMO split-window -dP -F '#{pane_id}' -t P:5.0)
...
printf 'stdin-stuff' | $TERMO split-window -d -I -t P:3.0
if [ $? -ne 0 ]; then
	echo "split-window -I failed."
	exit 1
fi
i=0
while out=$($TERMO capture-pane -p -t P:3.1 | sed -n 1p) && \
    [ "$out" != "stdin-stuff" ]; do
	i=$((i + 1))
...
sid=$($TERMO split-window -d -P -F '#{pane_id}' -s 'bg=red' -t P:3.0)
out=$($TERMO show-options -v -p -t "$sid" window-style)
if [ "$out" != "bg=red" ]; then
...
check_windows()
{
	out=$(echo $($TERMO list-windows -t "$1" -F \
...
out=$($TERMO break-pane -d -P -F '#{window_name}' -s Q:5.0 -n qmoved -t Q:)
if [ "$out" != "qmoved" ]; then
	echo "single-pane break-pane output wrong: '$out'"
	exit 1
fi
if $TERMO has-session -t Q:qsolo 2>/dev/null; then
...
i=0
while [ "$($TERMO display-message -p -t P:2.0 '#{history_size}')" != "77" ]
do
	i=$((i + 1))
