...
check_ok split-window -d -t R:0
p0=$($TERMO display-message -p -t R:0.0 '#{pane_id}')
p1=$($TERMO display-message -p -t R:0.1 '#{pane_id}')
p2=$($TERMO display-message -p -t R:0.2 '#{pane_id}')
...
check_panes()
{
	out=$(echo $($TERMO list-panes -t "$1" -F \
	    '#{pane_index}:#{pane_id}'))
	if [ "$out" != "$2" ]; then
...
layout=$($TERMO display-message -p -t R:0 '#{window_layout}')
...
# error.
out=$($TERMO new-window -d -t W:0 -n wa -P -F '#{window_index}:#{window_name}')
if [ "$out" != "0:wa" ]; then
...
if $TERMO has-session -t D 2>/dev/null; then
