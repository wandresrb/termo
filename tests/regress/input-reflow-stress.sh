...
assert_alive()
{
	$TERMO display-message -p -t stress: alive >/dev/null 2>&1 ||
...
load_and_paste()
{
	...
	"$@" >"$EXP"
	$TERMO load-buffer -b "$buffer" "$EXP" || exit 1
	$TERMO paste-buffer -d -b "$buffer" -t stress:0.0 || exit 1
	sleep 0.3
}

capture_joined()
{
	$TERMO capture-pane -pNJ -t stress: -S -5000 -E - >"$TMP"
}
...
assert_raw_sane()
{
	...
	$TERMO capture-pane -pR -t stress: >"$TMP" ||
	...
	width=$($TERMO display-message -p -t stress: '#{pane_width}' 2>/dev/null)
	case "$width" in
	''|*[!0-9]*) width=0 ;;
	esac
...
{
	label=$1
	size=$($TERMO display-message -p -t stress: '#{history_size}' 2>/dev/null)
	case "$size" in
	''|*[!0-9]*)
...
assert_copy_mode_sane()
{
	...
	$TERMO capture-pane -pM -S -5000 -E - -t stress: >"$TMP" ||
...
run_resize_checks()
{
	for width in $WIDTHS; do
		$TERMO resize-window -t stress: -x "$width" -y "$HEIGHT" || exit 1
		sleep 0.1
...
wait_outer_contains()
{
	...
	i=0
	while [ "$i" -lt 50 ]; do
		$TERMO2 capture-pane -p -t out:0 >"$TMP" 2>/dev/null &&
			grep -F "$marker" "$TMP" >/dev/null 2>&1 &&
			return 0
		sleep 0.2
		i=$((i + 1))
...
{
	$TERMO set-option -t stress: status on >/dev/null || exit 1
	$TERMO set-option -t stress: status-interval 1 >/dev/null || exit 1

	$TERMO2 kill-server 2>/dev/null
	$TERMO2 new-session -d -s out -x 100 -y 12 "$TERMO attach -t stress" ||
		exit 1

	i=0
	while [ "$i" -lt 50 ]; do
		clients=$($TERMO list-clients -F x 2>/dev/null | grep -c x)
		[ "$clients" -ge 1 ] && break
		sleep 0.2
		i=$((i + 1))
	done
	...
	for width in $LIVE_WIDTHS; do
		$TERMO set-option -t stress: status-left "REDRAW-$width " >/dev/null ||
			exit 1
		$TERMO2 resize-window -t out: -x "$width" -y 12 || exit 1
		sleep 0.2
	done
	...
	$TERMO2 kill-server 2>/dev/null
	$TERMO set-option -t stress: status off >/dev/null || exit 1
}
...
$TERMO set-option -g history-limit "$HISTORY_LIMIT" || exit 1
$TERMO new-session -d -x 80 -y "$HEIGHT" -s stress 'stty -echo; exec cat' || exit 1
$TERMO kill-session -t test-setup || exit 1
sleep 0.3
...
$TERMO resize-window -t stress: -x 40 -y "$HEIGHT" || exit 1
sleep 0.1
...
$TERMO copy-mode -H -t stress: || exit 1
$TERMO send-keys -t stress: -X history-top
sleep 0.1
assert_alive "copy-mode"
assert_copy_mode_sane "copy-mode history-top"
$TERMO send-keys -t stress: -X cancel
sleep 0.1
