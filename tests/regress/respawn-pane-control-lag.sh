...
alive() {
	...
	$TERMO has -t rt || exit 1
}

wait_clients() {
	n=0
	while [ $n -lt 50 ]; do
		[ "$($TERMO lsc 2>/dev/null | wc -l)" -ge $1 ] && return
		sleep 0.1
		n=$((n + 1))
	done
...
$TERMO -f/dev/null new -d -x 80 -y 24 -s rt \
	'while :; do echo xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx; sleep 0.01; done' ||
	exit 1
SERVER=$($TERMO display -pt rt '#{pid}')
...
( sleep 60 ) | $TERMO -f/dev/null -C attach -t rt >"$FIFO" 2>&1 &
exec 8<"$FIFO"
( sleep 60 ) | $TERMO -f/dev/null -C attach -t rt >/dev/null 2>&1 &
wait_clients 2
...
sleep 3
$TERMO respawn-pane -k -t rt:0 'echo respawned; sleep 60' || exit 1
sleep 1
...
$TERMO neww -d -t rt \
	...
	exit 1
sleep 1
$TERMO new -d -s other || exit 1
$TERMO movew -d -s rt:1 -t other: || exit 1
sleep 2
$TERMO respawn-pane -k -t other:1 'echo respawned; sleep 60' || exit 1
sleep 1
