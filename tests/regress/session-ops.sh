...
check_sessions()
{
	out=$(echo $($TERMO list-sessions -F '#{session_name}' | LC_ALL=C sort))
	if [ "$out" != "$1" ]; then
...
wait_client_session()
{
	...
	while [ $i -lt 30 ]; do
		out=$($TERMO list-clients -F '#{client_session}' 2>/dev/null ||
		    true)
...
attach_control_client()
{
	...
	mkfifo "$fifo" || exit 1
	$TERMO -C attach-session -t "$1" <"$fifo" >/dev/null 2>&1 &
	control_pid=$!
	exec 9>"$fifo"
	rm -f "$fifo"
...
check_attached_destroy()
{
	...
	cat >"$script" <<-EOF
	#!/bin/sh
	"$TEST_TERMO" -LtestInner$$-$mode -f/dev/null new \\; \
	...
	pane=$($TERMO capture-pane -pt "$outer:0.0" -S -)
	if [ ! -f "$rcfile" ]; then
		echo "Inner tmux did not exit."
		echo "$pane"
		$TERMO kill-session -t "$outer" 2>/dev/null || true
		rm -rf "$outdir"
		exit 1
	fi
	rc=$(cat "$rcfile")
...
oddid=$($TERMO new-session -d -s 'a:b.c' -x 80 -y 24 -P -F '#{session_id}')
check_fmt "$oddid" '#{session_name}' 'a:b.c'
check_ok kill-session -t "$oddid"
emptyid=$($TERMO new-session -d -s '' -x 80 -y 24 -P -F '#{session_id}')
...
autoname=$($TERMO new-session -d -x 80 -y 24 -P -F '#{session_name}')
autoid=$($TERMO display-message -p -t "=$autoname:" '#{session_id}')
...
id=$($TERMO display-message -p -t S2: '#{session_id}')
...
if $TERMO has-session -t S1 2>/dev/null; then
