...
OUT="$TEST_TERMO -LtestA$$ -f/dev/null"		# outer (host for the client)
IN="$TEST_TERMO -LtestB$$ -f/dev/null"		# inner (under test)
...
$IN display-message -p '#{version}' >/dev/null 2>&1 || fail "inner termo died"
