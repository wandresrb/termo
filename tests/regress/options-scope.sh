...
out=$($TERMO show -A 2>/dev/null | grep '^status-left\*')
...
out=$($TERMO show -Ap 2>/dev/null | grep '^cursor-colour ')
...
$TERMO split-window -t main || exit 1
panes=$($TERMO list-panes -t main -F '#{pane_id}')
set -- $panes
pa=$1
pb=$2
...
$TERMO set -g @listme "here" || exit 1
if ! $TERMO show -g | grep -q '^@listme here$'; then
	echo "show -g did not list @listme."
	exit 1
fi
# alert-bell is a hook: only shown with -H.
if $TERMO show -g | grep -q '^alert-bell'; then
	echo "show -g listed a hook without -H."
	exit 1
fi
if ! $TERMO show -gH | grep -q '^alert-bell'; then
