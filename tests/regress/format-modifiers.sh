...
test_format()
{
	...
	if [ -n "$target" ]; then
		out=$($TERMO display-message -t "$target" -p "$fmt")
	else
		out=$($TERMO display-message -p "$fmt")
	fi
...
test_expand()
{
	...
	out=$($TERMO list-windows -t tf -F "$fmt")
...
$TERMO new-session -d -s tf || exit 1
...
$TERMO set -g @s 'abcdefghij' || exit 1
$TERMO set -g @path '/usr/local/bin/foo' || exit 1
$TERMO set -g @name 'window-name' || exit 1
$TERMO set -g @greek 'αβγ' || exit 1   # 6 bytes, 3 columns wide
$TERMO set -g @cjk '中文' || exit 1     # 6 bytes, 4 columns wide
$TERMO set -g @host 'myhost' || exit 1
$TERMO set -g @ts '1000000000' || exit 1  # 2001-09-09 01:46:40 UTC
$TERMO set -g @sp 'a b$c' || exit 1     # shell-special characters for q:
$TERMO set -g @hash 'a#b' || exit 1     # a "#" for q/e:
$TERMO set -g @sq "a'b" || exit 1       # a single quote for q/s:
$TERMO set -g @sub 'abABab' || exit 1
$TERMO set -g @slash 'foo/bar foo/' || exit 1
$TERMO set -g @nl "$(printf 'a\nb')" || exit 1
q_s_nl=$(printf "'a\nb'")
...
$TERMO rename-window -t main:0 knownwin
...
$TERMO display-message -p "#{e|/|:5,0}" >/dev/null 2>&1
$TERMO display-message -p "#{e|/|f:5,0}" >/dev/null 2>&1
...
for age in 30 300 4000 90000 200000 3000000 40000000; do
	$TERMO set -g @age "$((now - age))"
	if [ -z "$($TERMO display-message -p '#{t/r:@age}')" ]; then
		echo "Empty #{t/r:@age} for age ${age}s"
		exit 1
	fi
	if [ -z "$($TERMO display-message -p '#{t/p:@age}')" ]; then
		echo "Empty #{t/p:@age} for age ${age}s"
		exit 1
	fi
done
# A time in the future has no relative form.
$TERMO set -g @future "$((now + 100000))"
test_format "#{t/r:@future}" ""
diff=$($TERMO display-message -p '#{t/d:@future}')
case "$diff" in
-[0-9]*) ;;
*)
...
$TERMO new-session -d -s search -x 80 -y 10 'cat'
sleep 1
$TERMO send-keys -t search: 'Zebra_Marker_42' Enter
sleep 1
...
$TERMO kill-session -t search 2>/dev/null
...
$TERMO set @rec '#{E:@rec}'
$TERMO display-message -p '#{E:@rec}' >/dev/null 2>&1
...
$TERMO set -g @emoji '😀😀' || exit 1   # 8 bytes, 4 columns
...
$TERMO show-messages >/dev/null 2>&1
...
for f in \
    ...
    '#{p6:@name}' \
    '#{=3:#{b:@path}}'; do
	$TERMO display-message -v -p "$f" >/dev/null 2>&1
done
...
$TERMO new-session -d -s zeta -x 80 -y 24 || exit 1
$TERMO new-session -d -s alpha || exit 1
$TERMO new-session -d -s mike || exit 1
$TERMO set -g automatic-rename off
...
$TERMO rename-window -t zeta:0 charlie
$TERMO new-window -d -t zeta:1 -n alpha
$TERMO new-window -d -t zeta:2 -n bravo
...
$TERMO split-window -h -t zeta:charlie
$TERMO split-window -h -t zeta:charlie
...
$TERMO new-pane -d -t zeta:charlie -x 20 -y 10 -X 1 -Y 1
...
$TERMO display-message -v -p "#{S:#{session_name}}" >/dev/null 2>&1
$TERMO display-message -v -t zeta: -p "#{W:#{window_name}}" >/dev/null 2>&1
$TERMO display-message -v -t zeta:charlie -p "#{P:#{pane_index}}" >/dev/null 2>&1
...
sleep 30 >"$FIFO2" &
HOLD2=$!
$TERMO -C attach -t zeta <"$FIFO1" >/dev/null 2>&1 &
CC1=$!
$TERMO -C attach -t alpha <"$FIFO2" >/dev/null 2>&1 &
CC2=$!
sleep 1
