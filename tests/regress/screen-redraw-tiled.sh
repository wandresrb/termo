...
new_scene() {
	...
	$TERMO2 kill-window -a 2>/dev/null
	$TERMO2 neww -d "$C" || exit 1
...
$TERMO2 set -g pane-border-format " #{pane_index} " || exit 1
...
for style in single double heavy simple number spaces none; do
	$TERMO2 set -g pane-border-lines $style || exit 1
...
$TERMO2 set -g pane-border-lines single || exit 1
...
$TERMO2 splitw -h "$C" || exit 1
$TERMO2 setw pane-border-format " #{pane_index}:#{pane_title} " || exit 1
$TERMO2 setw pane-border-status bottom || exit 1
$TERMO2 selectp -t:.0 -T left || exit 1
$TERMO2 selectp -t:.1 -T right || exit 1
$TERMO2 selectp -t:.0 || exit 1
...
title_all() {
	for p in $($TERMO2 list-panes -F '#{pane_index}'); do
		$TERMO2 selectp -t:.$p -T $1$p || exit 1
	done
	$TERMO2 selectp -t:.0 || exit 1
}
