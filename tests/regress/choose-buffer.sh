...
# second server provides a client: an inner "termo attach" runs inside a pane
...
wait_buffers()
{
	i=0
	while [ "$i" -lt 50 ]; do
		c=$($TERMO list-buffers -F x 2>/dev/null | grep -c x)
		[ "$c" -eq "$1" ] && return 0
		sleep 0.5
		i=$((i + 1))
	done
...
$TERMO set-buffer -b bufa "hello buffer" || exit 1
$TERMO set-buffer -b bufz "other buffer" || exit 1

# --- filter by buffer name ---------------------------------------------------
$TERMO choose-buffer -t aaa:0 -F 'B1' -f '#{==:#{buffer_name},bufa}' || exit 1
wait_count ': B1' 1
out=$CAPTURED
...
$TERMO choose-buffer -t aaa:0 -F 'B2' -f '#{m:*hello*,#{buffer_sample}}' || \
	exit 1
wait_count ': B2' 1
out=$CAPTURED
...
$TERMO choose-buffer -t aaa:0 -F 'B3' || exit 1
wait_count ': B3' 2
out=$CAPTURED
...
$TERMO choose-buffer -t aaa:0 -F 'B4' -f '#{==:#{buffer_name},nosuch}' || \
	exit 1
wait_count ': B4' 2
out=$CAPTURED
...
$TERMO choose-buffer -t aaa:0 -F 'B5' -O name || exit 1
wait_count ': B5' 2
...
$TERMO choose-buffer -t aaa:0 -F 'B6' -O name -r || exit 1
wait_count ': B6' 2
...
$TERMO choose-buffer -t aaa:0 -F 'G1' -f '#{==:#{buffer_name},bufz}' || exit 1
wait_for 'bufz: G1'
$TERMO send-keys -t aaa:0 d
wait_buffers 1
$TERMO list-buffers -F '#{buffer_name}' | grep -F -q 'bufa' || \
	fail "wrong buffer deleted"
exit_mode ': G1' q

# --- C-t tags all buffers and D deletes the tagged ------------------------------
$TERMO set-buffer -b bufz "other buffer" || exit 1
$TERMO set-buffer -b bufb "third buffer" || exit 1
$TERMO choose-buffer -t aaa:0 -F 'G2' || exit 1
wait_count ': G2' 3
$TERMO send-keys -t aaa:0 C-t D
wait_buffers 0
wait_mode aaa:0 0
wait_gone ': G2'
...
$TERMO set-buffer -b bufa "hello buffer" || exit 1
$TERMO choose-buffer -t aaa:0 -F 'G3' || exit 1
wait_count ': G3' 1
exit_mode ': G3' Enter
i=0
while [ "$i" -lt 50 ]; do
	$TERMO capture-pane -p -t aaa:0 | grep -F -q 'hello buffer' && \
		break
	sleep 0.2
	i=$((i + 1))
done
