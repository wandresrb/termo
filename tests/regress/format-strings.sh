...
test_conditional_with_pane_in_mode()
{
	...
	$TERMO copy-mode # enter copy mode
	test_format "$fmt" "$exp_true"
	$TERMO send-keys -X cancel # leave copy mode
...
test_conditional_with_session_name()
{
	...
	$TERMO rename-session "Summer"
	test_format "$fmt" "$exp_summer"
	$TERMO rename-session "Winter"
	test_format "$fmt" "$exp_winter"
	$TERMO rename-session "Summer" # restore default
}


$TERMO kill-server 2>/dev/null
$TERMO -f/dev/null new-session -d || exit 1

# used later in conditionals
$TERMO rename-session "Summer" || exit 1
$TERMO set @true 1 || exit 1
$TERMO set @false 0 || exit 1
$TERMO set @warm Summer || exit 1
$TERMO set @cold Winter || exit 1
$TERMO set @v 'foo:bar' || exit 1
...
#$TERMO display-message -p "#(echo #,)" > /dev/null
#test_conditional_with_pane_in_mode "#{?pane_in_mode,#(echo #,),xyz}" "," "xyz"
#test_conditional_with_pane_in_mode "#{?pane_in_mode,#(echo #,),xyz}" "," "xyz"
...
$TERMO rename-session ","
test_format "#{?#{==:#,,#{session_name}},abc,xyz}" "abc"
$TERMO rename-session "Summer" # reset to default
