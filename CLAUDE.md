...
**Lua specs** (`tests/lua/`): `meson test -C build --suite lua` runs `tests/lua/run.sh`, which starts a server on its own socket, executes `run.lua` through `run-lua -f`, waits on `wait-for lua-specs` and prints the TAP `spec_report()` collected. Specs use `describe/it/eq/fails`; `it_async(name, fn(done))` runs from a timer after the command returns, one spec at a time so sync `cmd()` calls see a free queue, and uses `check(done, cond, msg)`. The runner fails when a function in `termo.api.list()` has no spec. A new spec file goes in the list at the bottom of `run.lua`. `tests/unit/test_lua.c` covers the runtime without a server; `tests/integration/test_lua_ui.py` attaches a client on a pty and drives menus, prompts, popups, messages, the palette and a function key binding from `tests/integration/lua_ui_init.lua`.
